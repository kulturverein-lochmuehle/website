import type { EntryCollection } from '@sveltia/cms';

export const pages: EntryCollection = {
  name: 'pages',
  label: 'Seiten',
  label_singular: 'Seite',
  description: 'Sichtbare Seiten der Website mit ihrer Struktur und Inhalten.',
  folder: 'src/content/pages',
  create: true,
  summary: '{{title}}',
  fields: [
    {
      name: 'title',
      label: 'Titel',
      widget: 'string',
    },
    {
      name: 'sections',
      label: 'Abschnitte',
      label_singular: 'Abschnitt',
      widget: 'list',
      types: [
        {
          name: 'section',
          label: 'Abschnitt',
          widget: 'object',
          fields: [
            {
              name: 'title',
              label: 'Titel',
              widget: 'string',
            },
            {
              name: 'id',
              label: 'URL-Slug',
              widget: 'string',
            },
            {
              name: 'theme',
              label: 'Farbschema',
              widget: 'select',
              options: [
                {
                  label: 'Dunkel (anthrazit)',
                  value: 'dark',
                },
                {
                  label: 'Hell (cyan)',
                  value: 'light',
                },
              ],
            },
            {
              name: 'contents',
              label: 'Inhalte',
              label_singular: 'Inhalt',
              widget: 'list',
              types: [
                {
                  name: 'teaser',
                  label: 'Teaser',
                  widget: 'object',
                  fields: [
                    {
                      name: 'scope',
                      label: 'Umfang',
                      widget: 'select',
                      options: [
                        {
                          label: 'Alle Veranstaltungen',
                          value: 'chronicle:all',
                        },
                        {
                          label: 'Bisherige Veranstaltungen',
                          value: 'chronicle:past',
                        },
                        {
                          label: 'Kommende Veranstaltungen',
                          value: 'chronicle:upcoming',
                        },
                        {
                          label: 'Nächste Veranstaltung',
                          value: 'chronicle:next',
                        },
                      ],
                    },
                    {
                      name: 'title',
                      label: 'Titel',
                      widget: 'string',
                    },
                  ],
                },
                {
                  name: 'typo',
                  label: 'Typografie',
                  widget: 'object',
                  fields: [
                    {
                      name: 'heading',
                      label: 'Überschrift',
                      widget: 'object',
                      fields: [
                        {
                          name: 'text',
                          label: 'Text',
                          widget: 'text',
                        },
                        {
                          name: 'level',
                          label: 'Ebene',
                          widget: 'number',
                          default: 1,
                          min: 1,
                          max: 6,
                        },
                        {
                          name: 'isTitle',
                          label: 'Als Titel',
                          widget: 'boolean',
                          default: false,
                          required: false,
                        },
                        {
                          name: 'isRightAligned',
                          label: 'Rechts ausgerichtet',
                          widget: 'boolean',
                          default: false,
                          required: false,
                        },
                        {
                          name: 'isSticky',
                          label: 'Fixiert',
                          widget: 'boolean',
                          default: false,
                          required: false,
                        },
                      ],
                    },
                    {
                      name: 'text',
                      label: 'Text',
                      widget: 'object',
                      required: false,
                      fields: [
                        {
                          name: 'text',
                          label: 'Text',
                          widget: 'markdown',
                          required: false,
                          minimal: true,
                          buttons: ['link'],
                        },
                        {
                          name: 'style',
                          label: 'Variante',
                          widget: 'select',
                          required: false,
                          default: 'body',
                          options: [
                            {
                              label: 'Hervorgehoben',
                              value: 'lead',
                            },
                            {
                              label: 'Fließtext',
                              value: 'body',
                            },
                          ],
                        },
                      ],
                    },
                  ],
                },
              ],
            },
          ],
        },
      ],
    },
  ],
};
